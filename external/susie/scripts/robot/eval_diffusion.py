...
DUMMY_OBS = np.zeros((200, 200, 3), dtype=np.uint8) 
...
def main(_):
    ...
    while True:
        ...
        while not done:
            ...
            resized_image_obs = np.array(Image.fromarray(image_obs).resize((256, 256))).astype(np.uint8)
            ...
            if FLAGS.num_samples == 1:
                ...
                goal_idx, goal_images_info, _ = high_level_vf_filter(vf_agent, prompt, image_obs, image_goals)
