...
class StableContrastiveRLVfAgent(flax.struct.PyTreeNode):
    ...
    def update(self, batch: Batch, pmap_axis: str = None):
        ...
        def actor_loss_fn(params, rng):
            ...
            q_action = self.state.apply_fn(
                {"params": self.state.params},  # no gradient flows through here
                (batch["observations"], batch["goals"]),
                jnp.zeros_like(sampled_actions), ### This will prevent the policy from learning anything 
                train=self.config["dropout_target_networks"],
                rngs={"dropout": key},
                name="critic",
            )
    ...
    def value_function(self, observations, goals):
        act_dim = 7 # For CALVIN

        batch_size = observations["image"].shape[0]
        actions = jnp.zeros((batch_size, act_dim), dtype=float)
        

        # observations["image"].shape: (batch_size, 200, 200, 3)
        # goals["image"].shape: (batch_size, 200, 200, 3)

        # logits.shape: (16, 16, 2)
        logits = self.state.apply_fn(
            {"params": self.state.params},
            (observations, goals),
        ...
        logits = logits.mean(-1) # (batch_size, batch_size)

        I = jnp.eye(batch_size)
        # logits_pos = jnp.sum(logits * I) / jnp.sum(I) # scalar
        return (logits * I).sum(axis=0) # (batch_size,) # get the elements along the diagonal. sum along axis=0 or axis=1, it's the same
    ...
    def get_debug_metrics(self, batch, **kwargs):
        ...
        correct = jnp.argmax(logits, axis=1) == jnp.argmax(I, axis=1)
        logits_pos = jnp.sum(logits * I) / jnp.sum(I) # scalar
        logits_neg = jnp.sum(logits * (1 - I)) / jnp.sum(1 - I) # scalar
    ...
    @jax.jit
    def get_debug_metrics2(self, batch, **kwargs):
        batch_size = batch["terminals"].shape[0]
    ...
    def create(
        ...
        params = model_def.init(
            ...
            critic=[(observations, goals), jnp.zeros_like(actions)],
        )["params"]
